...
def obtain_traced_model_dict(model):
    ...
    elif type(model) in [furiosa_llm_models.llama.symbolic.mlperf_submission.LlamaForCausalLM, furiosa_llm_models.llama.symbolic.mlperf_submission_slice.LlamaForCausalLM]:
...
def get_generator_for_submission_model_slice(model_path, qconfig_path, submission_quant_param_path, submission_quant_format_path, gpu, n_layers, logit_folder_path, mcp_dumping_on):
    ...
    return PagedAttentionGenerator(**quant_submission_models, bucket_size = BUCKET_SIZE, batch_size = BATCH_SIZE, is_2d_attention=True, kv_dtype = QUANT_KV_DTYPE)
...
def perform_generation(generator, test_data_list, logit_file_path, generation_result_file_path, tokenizer):
    ...
    with torch.no_grad():
        for idx, test_data in enumerate(test_data_list):
            ...
            elif type(generator) == PagedAttentionGenerator:
                    output = generator.generate(**test_data, max_length = test_data['input_ids'].shape[1] + gen_kwargs["max_new_tokens"])
...
def compare_model_outputs(args):
    ...
    submission_model_generator = get_generator_for_submission_model_slice(args.model_path,
                                                        args.quant_config_path, 
